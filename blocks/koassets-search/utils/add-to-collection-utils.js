...
/**
 * Check if an asset is a template asset
 * @param {Object} asset - The asset object
 * @returns {boolean} True if the asset is a template asset
 */
export function isTemplateAsset(asset) {
  const templatePath = asset['tccc-templatePath'] || asset.templatePath;
  return Boolean(templatePath && templatePath !== 'N/A');
}

/**
 * Generate the HTML for the add-to-collection overlay
 * @param {Object} asset - The asset object
 * @returns {string} HTML string for the overlay, or empty string if template asset
 */
export function getAddToCollectionOverlayHTML(asset) {
  if (isTemplateAsset(asset)) {
    return '';
  }
