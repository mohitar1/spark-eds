...
import { cleanBrandName } from '../../../scripts/analytics/brand-utils.js';
import { detectResourceType } from '../../../scripts/analytics/resource-type-detector.js';
import { UNKNOWN_VALUE, DOWNLOAD_TYPES } from '../../../scripts/analytics/analytics-constants.js';
...
const FACET_BUCKET_SIZE = 250;

/**
 * Determine download type based on asset rights status
 * @param {Object} asset - Asset object with readyToUse field
 * @returns {string} Download type ('ready-to-use', 'restricted', or 'unknown')
 */
export function getDownloadType(asset) {
  const readyToUse = asset?.readyToUse?.toLowerCase()?.trim();

  if (readyToUse === 'yes') {
    return DOWNLOAD_TYPES.READY_TO_USE;
  }
  if (readyToUse === 'no') {
    return DOWNLOAD_TYPES.RESTRICTED;
  }
  // Missing, empty, or unexpected value - flag for data quality
  return DOWNLOAD_TYPES.UNKNOWN;
}
...
// Facet keys that need hidden versions for value mapping lookups
// Populated dynamically from facets config where getLabelFromHidden === true
let hiddenValueMappingFacetKeys = [];

/**
 * Set the facet keys that need hidden versions for value mapping lookups
 * @param {string[]} keys - Array of facet keys
 */
export function setHiddenValueMappingFacetKeys(keys) {
  hiddenValueMappingFacetKeys = keys;
}

/**
 * Get the facet keys that need hidden versions for value mapping lookups
 * @returns {string[]} Array of facet keys
 */
export function getHiddenValueMappingFacetKeys() {
  return hiddenValueMappingFacetKeys;
}
...
export class DynamicMediaClient {
  /**
   * Current download session ID for grouping related downloads
   * Set via startDownloadSession() before batch downloads
   * @type {string|null}
   */
  currentDownloadId = null;

  /**
   * Start a new download session for grouping multiple downloads
   * Call before a batch of related downloads
   * @returns {string} The session ID for this download batch
   */
   ...
   * Call after a batch of related downloads is complete
   */
  endDownloadSession() {
   ...
   * @private
   * @param {string} filter - Facet filter (e.g., 'tccc-subBrand:tccc:brand/fanta/fanta-orange')
   *   Keys are in hyphen format (namespace-fieldName), values may contain colons
   * @returns {{ key: string, value: string }} Base key (hyphen format) and value
   */
  // eslint-disable-next-line class-methods-use-this
  parseFacetFilter(filter) {
    // Keys are always in format 'namespace-fieldName' (with hyphen)
    // Replace first hyphen with colon, then split to extract key and value
    const hyphenIndex = filter.indexOf('-');

    if (hyphenIndex === -1) {
      // No hyphen - key has no namespace, split on first colon
      const colonIndex = filter.indexOf(':');
      if (colonIndex === -1) return { key: filter, value: '' };
      return { key: filter.substring(0, colonIndex), value: filter.substring(colonIndex + 1) };
    }

    // Replace first hyphen with colon to normalize for splitting
    const normalized = `${filter.substring(0, hyphenIndex)}:${filter.substring(hyphenIndex + 1)}`;

    // Split by colon - first two tokens are the key (namespace:field), rest is value
    const parts = normalized.split(':');
    if (parts.length < 2) return { key: filter, value: '' };

    // Return key in original hyphen format for consistency with facet IDs
   ...
   * Field may contain colons (e.g., tccc:contentType), so split at LAST colon
   * Field may use hyphens (e.g., tccc-contentType) or colons (tccc:contentType)
  ...
  parseTerm(tokens, ctx) {
    ...
    // e.g., 'tccc:contentType:marketing' -> field='tccc:contentType', value='marketing'
    // e.g., 'tccc-contentType:marketing' -> field='tccc-contentType', value='marketing'
    ...
    // Normalize field to hyphen format for lookup (tccc:contentType -> tccc-contentType)
   ...
   * Examples:
   *   'tccc-contentType:marketing OR tccc-contentType:customers'
   *   '(tccc-brand:Coca-Cola AND tccc-market:US) OR tccc-brand:Sprite'
   *   'NOT tccc-contentType:internal'
   * @private
  ...
  buildQueryArray(query, options) {
    ...
    // Add preset filters (e.g., 'tccc-contentType:marketing OR tccc-contentType:customers')
    ...
    if (facetFilters.length > 0) {
      ...
      flatFilters.forEach((filter) => {
        // Skip non-string values (safety check)
        if (typeof filter !== 'string') return;

        const { key, value } = this.parseFacetFilter(filter);
        if (key && value) {
  ...
  buildFacetsArray(facetKeys, facetFilters = [], numericFilters = []) {
    ...
    facetKeys.forEach((facetKey) => {
      ...
      if (isDateFacet) {
        ...
        // Create hidden version for specific facets (for value mapping lookups)
        // Hidden facet should NOT have selections - it's only for display name lookups
        if (getHiddenValueMappingFacetKeys().includes(facetKey)) {
          const hiddenFacet = {
            type: 'CATEGORY',
            id: `${facetKey}_hidden`,
            field: `${contentAIField}_hidden`,
            size: FACET_BUCKET_SIZE,
            sort: 'COUNT_DESC',
          };

          // Hidden facet also gets scope from other facets
          if (scope) {
            hiddenFacet.scope = scope;
          }

          facets.push(hiddenFacet);
        }
      }
    });
   ...
   * @param {string} [ifNoneMatch] - ETag for conditional request
  ...
  async downloadAsset(asset, rendition = { name: ORIGINAL_RENDITION }, isImagePreset = false) {
    ...
    // Add analytics parameters for server-side tracking
    let resourceType = 'asset';
    try {
      const detection = detectResourceType(asset);
      resourceType = detection.type;
    } catch (error) {
      // eslint-disable-next-line no-console
      console.error('[Download Analytics] Resource type detection failed:', error);
      // Fallback to 'asset'
    }

    // Generate downloadId for session grouping (or use existing if set on client)
    const downloadId = this.currentDownloadId || generateUUID();

    const analyticsParams = {
      'x-analytics-brand': cleanBrandName(asset?.brand),
      'x-analytics-campaign': asset?.campaignName || UNKNOWN_VALUE,
      'x-analytics-resource-type': resourceType,
      // New enhanced tracking fields
      'x-analytics-download-id': downloadId,
      'x-analytics-item-id': asset?.assetId || '',
      'x-analytics-download-type': getDownloadType(asset),
      'x-analytics-rendition': rendition?.name || ORIGINAL_RENDITION,
    };
    ...
    Object.entries(analyticsParams).forEach(([key, value]) => {
      if (value) {
        searchParams.append(key, String(value));
      }
    });
  ...
  /**
   * Create assets archive with analytics tracking
   * @param {Array<{asset: Object, renditions: Array}>} assetRenditionPairs - Assets with renditions
   * @param {Object} [options] - Optional configuration
   * @param {string} [options.downloadId] - Session ID to group all assets in this archive
   * @returns {Promise<string|null>} Archive ID or null
   */
  async createAssetsArchive(assetRenditionPairs, options = {}) {
    try {
      // Generate or use provided downloadId to group all assets in this archive
      const downloadId = options.downloadId || this.currentDownloadId || generateUUID();
      ...
      // Build analytics context for server-side tracking
      // Server will track each asset in the archive with the same downloadId
      const analyticsContext = {
        downloadId,
        assets: assetRenditionPairs.map((pair) => {
          const { asset, renditions } = pair;
          // Ensure renditions is an array and filter out empty/undefined names
          const renditionNames = (renditions || [])
            .map((r) => r?.name || 'original')
            .filter((name) => name && typeof name === 'string');
          return {
            assetId: asset?.assetId || '',
            brand: cleanBrandName(asset?.brand),
            campaign: asset?.campaignName || UNKNOWN_VALUE,
            downloadType: getDownloadType(asset),
            // Ensure at least 'original' if no renditions
            renditions: renditionNames.length > 0 ? renditionNames : ['original'],
          };
        }),
      };
      ...
      const responseData = await this.makeRequest({
        ...
        data: payload,
        headers: {
          'x-analytics-context': JSON.stringify(analyticsContext),
        },
        allowUndefinedResponse: true,
      });
  ...
  downloadFromUrl(url, defaultFilename = 'download') {
    try {
      // eslint-disable-next-line no-console
      console.log('[Download Analytics] downloadFromUrl called:', {
        url,
        defaultFilename,
        note: 'This is a direct URL download (archive) - no asset metadata available',
      });
