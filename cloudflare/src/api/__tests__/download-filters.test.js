...
describe('buildDownloadFilterConditions', () => {
  ...
  describe('Role Filter', () => {
    it('should filter by associate role with mappings', () => {
      ...
      expect(result).toBe(" AND (blob5 LIKE '%associate%' OR blob5 LIKE '%employee%' OR blob5 LIKE '%contingent-worker%')");
    });
    ...
    it('should throw error for invalid role value (SQL injection prevention)', () => {
      expect(() => buildDownloadFilterConditions({ role: "'; DROP TABLE users; --" }))
        .toThrow('Invalid role filter');
    });

    it('should throw error for SQL injection attempt in role', () => {
      expect(() => buildDownloadFilterConditions({ role: "admin' OR '1'='1" }))
        .toThrow('Invalid role filter');
    });
  });
  ...
  describe('Region Filter', () => {
    ...
    it('should throw error for invalid region value (SQL injection prevention)', () => {
      expect(() => buildDownloadFilterConditions({ region: "XX'; DROP TABLE users; --" }))
        .toThrow('Invalid region filter');
    });

    it('should throw error for SQL injection attempt in region', () => {
      expect(() => buildDownloadFilterConditions({ region: "' OR '1'='1" }))
        .toThrow('Invalid region filter');
    });
  });
  ...
  describe('SQL Injection Prevention', () => {
    it('should reject role with SQL injection attempt', () => {
      expect(() => buildDownloadFilterConditions({ role: "' OR 1=1 --" }))
        .toThrow('Invalid role filter');
    });
    ...
    it('should reject role with UNION attack', () => {
      expect(() => buildDownloadFilterConditions({ role: "' UNION SELECT * FROM users --" }))
        .toThrow('Invalid role filter');
    });

    it('should reject region with semicolon', () => {
      expect(() => buildDownloadFilterConditions({ region: "NA; DROP TABLE" }))
        .toThrow('Invalid region filter');
    });

    it('should reject role with script tags', () => {
      expect(() => buildDownloadFilterConditions({ role: '<script>alert("xss")</script>' }))
        .toThrow('Invalid role filter');
    });
  });
